...
{
    if(!NET_DEVICE_IS_UP(dev)) {                     //デバイスの状態を確認(UP状態でなければ送信できないためエラーを返す)
        errorf("not opened, dev=%s", dev->name);
        return -1;
    }
    if(len > dev->mtu) {                            //データサイズの確認
        errorf("too long, dev=%s, mtu=%u, len=%zu", dev->name, dev->mtu, len);
        return -1;
    }
    debugf("dev=%s, type=0x%04x, len=%zu", dev->name, type, len);
    debugdump(data, len);
    if (dev->ops->transmit(dev, type, data, len, dst) == 1) {  //デバイスドライバの出力関数を呼び出す
...
void
net_shutdown(void)  //登録済みの全デバイスをクローズ
{
    struct net_device *dev;
